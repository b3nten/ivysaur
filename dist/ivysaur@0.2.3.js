...
function onEffectCleanup(fn, failSilently = false) {
  if (activeSub instanceof ReactiveEffect) {
    ...
    warn(
      `onEffectCleanup() was called when there was no active effect to associate\
 with.`
    );
  }
}
...
var patch_node = (parent, node, oldVNode, newVNode, isSvg) => {
  ...
  TEXT_NODE) {
    ...
    patch_props(node, oldVNode.props, newVNode.props, isSvg = isSvg || newVNode.
    tag === "svg");
    patch_children(node, oldVNode, newVNode, isSvg);
  }
      ...
      return {
        ...
        set(val) {
          ...
          if (!val) {
            ...
            this.setAttribute(
              attrName,
              typeof val === "object" ? JSON.stringify(val) : typeof val === true ?
              "" : String(val)
            );
          }
        },
