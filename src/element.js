...
export { Ivysaur, Ivysaur as ivysaur, css, state, attribute, set_dev, set_dev as setDev, IS_DEV, h, render, Fragment }
  ...
  */
  ...
  constructor() {
    ...
    if (IS_DEV && !has_warned_dev) {
      console.warn("Ivysaur is running in development mode. Call set_dev(false) to disable this warning.")
      has_warned_dev = true;
    }
  }
...
function attribute(overriddenName, options = {}) {
  return function (value, { kind, name, metadata }) {
    ...
    if (kind === "accessor") {
      ...
      throw new Error("Invalid decorator usage: @attribute only works on class accessors and getters.");
    }
  }
}
